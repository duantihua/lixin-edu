 ...
 */
object LixinPlanExcelParser {

  def main(args: Array[String]): Unit = {
    if (args.length < 1) {
      println("Using LixinPlanExcelParser file")
      return
    }
    val file = args(0)
    val reader = new LixinPlanExcelParser(new FileInputStream(file))
    reader.process()
  ...
  class ExcelPlan(val name: String) {
    ...
    private def convertGroup(g: LixinPlanExcelParser.ExcelCourseGroup, program: Program, entityDao: EntityDao, messages: mutable.Buffer[String]): Option[MajorCourseGroup] = {
  ...
  class ExcelPlanCourse(val seq: Int, val courseCode: String, val courseName: String, val courseNameEn: String, val credits: Double,
    ...
    def toTerms: Terms = LixinPlanExcelParser.toTerms(terms)
...
import net.openurp.lixin.edu.program.service.LixinPlanExcelParser.*

class LixinPlanExcelParser(in: InputStream) extends Logging {
